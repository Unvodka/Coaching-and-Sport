...
interface CoachProgramsClientProps {
  initialPrograms: ProgramWithAssignments[];
  initialUsers: SimpleUser[];
}

export default function CoachProgramsClient({
  initialPrograms,
  initialUsers,
}: CoachProgramsClientProps) {
  ...
  const usersWithPrograms = initialUsers.filter((u) =>
        ...
        <div className="space-y-3">
          {filteredPrograms.map((program) => {
            ...
            const assignedNames = program.assigned_user_ids
              .map((uid) => {
                const u = initialUsers.find((user) => user.id === uid);
